...
class DIV2K:
    def __init__(self,
        ...
        if scale in _scales:
            ...
            raise ValueError('scale must be in ${}'.format(_scales))
    ...
    def hr_dataset(self):
        if not os.path.exists(self._hr_images_dir()):
            print('{} not found, downloading it...'.format(self._hr_images_dir()))
    ...
    def lr_dataset(self):
        if not os.path.exists(self._lr_images_dir()):
            print('{} not found, downloading it...'.format(self._lr_images_dir()))
    ...
    def _lr_cache_file(self):
        return os.path.join(self.caches_dir, 'DIV2K_{}_LR_{}_X{}.cache'.format(self.subset,
        self.downgrade, self.scale))
    ...
    def _lr_image_file(self, image_id):
        if not self._ntire_2018 or self.scale == 8:
            return '{:04}x{}.png'.format(image_id, self.scale)
        else:
            return '{:04}x{}{}.png'.format(image_id, self.scale, self.downgrade[0])
